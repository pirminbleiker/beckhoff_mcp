...
```mermaid
graph TB
    subgraph CLIENT[Client pyads oder AdsClient]
        APP[Application Code]
        AMS_LIB[AMS Layer baut Frame mit Source und Target NetId und Cmd]
    end

    subgraph ROUTER[Router Service TC1000 Equivalent]
        IPC_IN[Local IPC akzeptiert AMS Frames]
        ROUTING[Routing Decision Target NetId nach StaticRoutes xml]
        TX_TCP[TCP Transport Adapter wraps AMS in TCP Frame]
        TX_MQTT[MQTT Transport Adapter publish auf Topic AdsOverMqtt target ams]
    ...
    subgraph REMOTE[Remote TwinCAT Runtime]
        REMOTE_RT[Remote Router unwraps Transport]
        AMS_TARGET[AMS Frame an lokalen Server Port 851]
        TC_RT[TwinCAT Runtime processiert ADS Command]
    end
...
```mermaid
graph LR
    subgraph LIN[Linux oder WSL]
        PYL[pyads Python] -->|TCP Loopback| ADSL[adslib so bundled]
        ADSL -->|TCP 48898| ROUTER1[Router Service]
    end

    subgraph WTC[Windows mit TwinCAT]
        PYW1[pyads Python] -->|Win32 Window Msg| TCD1[TcAdsDll dll]
        TCD1 -->|IPC TcAmsWindow| TCSYS[TcSystemServiceUm exe]
    end

    subgraph WONLY[Windows ohne TwinCAT]
        PYW2[pyads Python] -->|Win32 Window Msg| TCD2[TcAdsDll dll]
        TCD2 -.findet kein.-> X[TcAmsWindow]
        X -.- LUECKE[LUECKE kein TC Service]
    end
...
Linux WSL pyads spricht bereits TCP AMS, kompatibel mit unserem Router. Windows pyads ohne TwinCAT geht prinzipiell nicht da TcAdsDll braucht Win32 Window.
...
```mermaid
sequenceDiagram
    participant TC as AdsTestClient
...
Code in AdsTestClient Program cs. Beweis dass MQTT Payload Wrapping und Topic Schema funktioniert.
...
graph TB
    subgraph WHOST[Windows Host]
        PY[pyads via WSL TCP Loopback]

        subgraph BRIDGE[AdsRouter Custom]
            TCPL[AmsTcpIpRouter TCP Loopback 48898]
            INT[Frame Interceptor liest Target NetId aus AMS Header]
            ROUTES[StaticRoutes xml Lookup]
            TCPADAPT[TCP Adapter im AmsTcpIpRouter]
            MQTTADAPT[MQTT Adapter nutzt AdsOverMqtt Library Calls]
        end
    end
...
Realistische Option: Eigener TCP Listener auf 48898 plus AMS Frame Parser plus per Route Type Dispatcher. AdsOverMqtt Plugin als Library nutzen, nicht als MEF Plugin.
...
| Lokale Server Endpoints Port 1 und 10000 MQTT faehig | OK |
| Forwarding fremder NetIds via MQTT | FEHLT Custom Code |
| pyads to Router to MQTT to PLC End to End | wartet auf MqttRouteHandler |
| NET AdsClient to MQTT to PLC | OK Durchstich |
...
4. MQTT Adapter schreiben (Publish AdsOverMqtt target ams, Subscribe self ams Hash)
